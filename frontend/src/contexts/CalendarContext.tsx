...
export function CalendarProvider({ children, year }: CalendarProviderProps) {
  ...
  const _unused_processEphemerisData = useCallback(
    (
      ...
      // Process each sample
      console.log(
        `Processing ${samples.length} ephemeris samples for station detection`
      );
        ...
        planetNames.forEach((planetName) => {
          ...
          if (
            ...
            if (i < 5 && planetName === "mercury") {
              console.log(
                `Sample ${i} - ${planetName} calculated speed: ${currentSpeed.toFixed(
                  4
                )} deg/day (from API: ${planet.speed})`
              );
            }
          ...
          const prevSpeedSign =
            prevState.speed !== null ? Math.sign(prevState.speed) : 0;
          ...
          if (speedCrossedZero) {
            ...
            const stationType: "retrograde" | "direct" =
              prevState.speed > 0 && currentSpeed < 0 ? "retrograde" : "direct";
          ...
          for (let j = i + 1; j < planetNames.length; j++) {
            ...
            for (const aspectType of aspectTypes) {
              ...
              if (
                ...
                const currentAspectAngle =
                  (((planet1.longitude - planet2.longitude) % 360) + 360) % 360;

                // Get previous aspect angle if available
                let prevAspectAngle: number | undefined;
                if (
                  prevAspectState &&
                  prevAspectState.orb !== undefined &&
                  prevAspectState.orb !== 999
                ) {
                ...
                if (i > 0 && samples[i - 1]?.planets) {
                  ...
                  if (prevP1 && prevP2) {
                    actualPrevAspectAngle =
                      (((prevP1.longitude - prevP2.longitude) % 360) + 360) %
                      360;
                  }
                }
      ...
      return {
        ...
        refinementData: eventsWithRefinement,
        samples,
      };
    },
    [currentChart]
    ...
    async (
      ...
      const refinedEvents = await Promise.all(
        events.map(async (event) => {
          ...
          const prevSample =
            refinement.prevSampleIndex !== undefined
              ? samples[refinement.prevSampleIndex]
              : null;
          const currentSample =
            (refinement as any).currentSampleIndex !== undefined
          ...
          const currentTime =
            currentSample.date || new Date(currentSample.timestamp);
          ...
          try {
            ...
            if (event.type === "ingress") {
              const targetSign =
                event.toSign === "Aries"
                  ...
                  : event.toSign === "Libra"
                  ? 6
                  : event.toSign === "Scorpio"
                  ? 7
                  : event.toSign === "Sagittarius"
                  ? 8
                  : event.toSign === "Capricorn"
                  ? 9
                  : event.toSign === "Aquarius"
                  ? 10
                  : 11;
              ...
              const currAngle =
                currPlanet1 && currPlanet2
                  ? getAngularDistance(
                      currPlanet1.longitude,
                      currPlanet2.longitude
                    )
                  : 0;
            ...
            console.error(
              `Error refining timestamp for event ${event.id}:`,
              error
            );
